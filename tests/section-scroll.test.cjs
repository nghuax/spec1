...
const source = fs.readFileSync('code/js/section-scroll.js', 'utf8').replace(/^import .*;\s*/m, '').replace('export class SectionScroll', 'globalThis.SectionScroll = class SectionScroll');
const motionSource = fs.readFileSync('code/js/motion.js', 'utf8').replaceAll('export ', '');
...
function fixture({ reduced = false, height = 900, introductions = false } = {}) {
  ...
  if(introductions) {
    const introSource=fs.readFileSync('code/js/stage-intros.js','utf8').replaceAll('export ','');
