...
const source=fs.readFileSync('code/js/gallery-frame-2026-09-22-v01.js','utf8').replace('export function','function');
...
const bridge=fs.readFileSync('code/artworks/gallery-bridge-2026-09-22-v01.js','utf8');
test('LIVEN bridge reports puzzle completion and preserves standalone isolation',()=>{
 ...
 const env={parent,location:{search:'?gallery=heal',pathname:'/code/artworks/liven/index-2026-09-22-v01.html',origin:'http://localhost'},URLSearchParams,document:{querySelector:()=>({}),getElementById:()=>({click:()=>count=0})},renewableCount:()=>count,earthIsRestored:()=>count===3,livenUI:{sync(){}},soundMuted:false,toggleSoundMute(){env.soundMuted=!env.soundMuted;},setInterval:f=>{env.tick=f;return 1;},clearInterval(){}};
...
test('EXHAUST bridge calls its p5 mixer handler and reset directly',()=>{
 ...
 const env={parent,location:{search:'?gallery=heal',pathname:'/code/artworks/exhaust/index-2026-09-22-v01.html',origin:'http://localhost'},URLSearchParams,document:{querySelector:()=>({})},projectMuted:false,ambienceButton:{},openSoundMixer:()=>opened++,regenerateArtwork:()=>resets++,setInterval:()=>1,clearInterval(){},window:{addEventListener:(n,f)=>listeners[n]=f}};
