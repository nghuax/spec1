...
test('production entry points resolve assets with Linux/GitHub Pages filename casing',()=>{
  const pending=['index-2026-09-22-v01.html','about-2026-09-22-v01.html',...['harm','exhaust','adapt','liven'].flatMap(id=>[`works/${id}-2026-09-22-v01.html`,`artworks/${id}/index-2026-09-22-v01.html`])];
  const seen=new Set();
...
test('all four exact chapter briefs and directional statements survive the final refinement',()=>{
  ...
  const source=fs.readFileSync(path.join(root,'js/stage-intros-2026-09-22-v01.js'),'utf8').replaceAll('export ','');
  ...
  const html=fs.readFileSync(path.join(root,'index-2026-09-22-v01.html'),'utf8').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ');
