...
function chapterHref(root, chapter) {
  return route(root, `works/${chapter.id}.html`);
}
...
function renderHome() {
  ...
  host.innerHTML = `
    ...
    <section class="home-about" data-reveal>
      ...
      <a class="text-link" href="${route(root, 'about.html')}">ABOUT HEAL <span aria-hidden="true">→</span></a>
    </section>`;
}
...
function adjacentLink(root, chapter, direction) {
  if (!chapter && direction === 'previous') {
    return `<a class="chapter-nav__link chapter-nav__link--previous" href="${route(root, 'index.html')}">
      <span>PREVIOUS</span><strong>← HEAL</strong></a>`;
  }

  if (!chapter && direction === 'next') {
    return `<a class="chapter-nav__link chapter-nav__link--next" href="${route(root, 'about.html')}#call-to-action">
  ...
  host.innerHTML = `
    ...
    <section class="credits" id="credits" aria-labelledby="credits-title">
      ...
      <div>
        ...
        <p>The site uses HTML, CSS, JavaScript and isolated p5.js artworks. Dependency notices and project licensing are included with the source.</p>
