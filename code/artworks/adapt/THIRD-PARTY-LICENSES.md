# Third-party licences

## p5.js

- Version: 1.9.0
- Copyright: Processing Foundation and contributors
- Licence: GNU Lesser General Public License 2.1
- Included file: `p5.js`
- Official project: <https://github.com/processing/p5.js>
- Official licence: <https://github.com/processing/p5.js/blob/main/license.txt>

The local runtime copy is included so the assessment can run without a network connection. No ownership claim is made over p5.js.

## Stack Sans Notch

- Typeface: Stack Sans Notch
- Designer: Koto / Stack Sans Project Authors
- Licence: SIL Open Font License 1.1
- Included file: `assets/fonts/stack-sans-notch-variable.woff2`
- Bundled licence: `assets/fonts/OFL.txt`
- Official source: <https://github.com/DylanYoungKoto/Stack-Sans>

These dependencies are separate from the GPL 3.0 licence applied to the student project code.

